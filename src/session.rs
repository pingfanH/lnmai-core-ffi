...
fn mk_lean_string(content: &str) -> *mut lean_object {
    ...
    unsafe { lean_mk_string(c.as_ptr()) }
}
...
impl Session<Empty> {
    pub fn create() -> Result<Self> {
        let json = unsafe { into_string(lnmai_create_empty_session_handle()) };
        if !is_ok(&json) {
    ...
    pub fn load_chart_text(self, content: &str, level_index: u32) -> Result<(Session<Loaded>, FfiEnvelope)> {
        ...
        let json = unsafe { into_string(lnmai_load_chart_into_session_from_text(self.handle, content_obj, level_index)) };
    ...
    pub fn load_chart_json(self, chart_spec_json: &str) -> Result<(Session<Loaded>, FfiEnvelope)> {
        ...
        let json = unsafe { into_string(lnmai_load_chart_into_session_from_json(self.handle, chart_obj)) };
        let envelope = ok_or_error(json)?;
        Ok((Session { handle: self.handle, _state: PhantomData }, envelope))
    }

    pub fn free(self) -> Result<FfiEnvelope> {
        let json = unsafe { into_string(lnmai_free_game_state_handle(self.handle)) };
        ok_or_error(json)
    }
}
...
impl Session<Loaded> {
    pub fn get_lowered_chart_json(&self) -> Result<FfiEnvelope> {
        let json = unsafe { into_string(lnmai_get_lowered_chart_json_by_handle(self.handle)) };
        ok_or_error(json)
    }

    pub fn advance_frame_light(&mut self, batch_json: &str) -> Result<FfiEnvelope> {
        let batch_obj = mk_lean_string(batch_json);
        let json = unsafe { into_string(lnmai_step_game_state_handle_light(self.handle, batch_obj)) };
        ok_or_error(json)
    }

    pub fn advance_frame_full(&mut self, batch_json: &str) -> Result<FfiEnvelope> {
        let batch_obj = mk_lean_string(batch_json);
        let json = unsafe { into_string(lnmai_step_game_state_handle(self.handle, batch_obj)) };
        ok_or_error(json)
    }

    pub fn get_state_json(&self) -> Result<FfiEnvelope> {
        let json = unsafe { into_string(lnmai_get_game_state_json_by_handle(self.handle)) };
        ok_or_error(json)
    }

    pub fn unload_chart(self) -> Result<(Session<Empty>, FfiEnvelope)> {
        let json = unsafe { into_string(lnmai_unload_chart_from_session(self.handle)) };
        let envelope = ok_or_error(json)?;
        Ok((Session { handle: self.handle, _state: PhantomData }, envelope))
    }
