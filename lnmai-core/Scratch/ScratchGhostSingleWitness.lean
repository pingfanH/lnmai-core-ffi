...
private def witnessFor (targets : List Nat) : List TimingSkeletonOverride :=
  targets.map (fun noteIndex => { noteIndex := noteIndex, resolve := tapHeadSensorAndReleaseNextFrame })
...
#eval do
  ...
  | .ok chart =>
      for targets in [[345], [458], [534], [345,458], [345,534], [458,534], [345,458,534]] do
        let result := simulateChartSpecWithTactic chart (tacticFromChartWithOverrides chart (witnessFor targets))
        IO.println s!"targets={targets} ap={achievesAP result} bads={nonPerfects result}"
